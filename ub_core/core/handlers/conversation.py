...
CONVO_FILTER = valid_chat_filter & create(
    lambda _, __, m: (m.chat.id in Conversation.CONVO_DICT.keys()) and (not m.reactions)
)


@bot.on_message(filters=CONVO_FILTER, group=0, is_command=False, filters_edited=True)
